...
function usePayFeesWithAwait({ fee, shouldPayBridgingFee }: Params) {
  const { api, isApiReady } = useApi();
  const { account } = useAccount();
  const { token } = useBridgeContext();

  const payFee = usePreparePayVaraFee();
  const signAndSend = useSignAndSend({ programs: [payFee.program] });
  ...
  const payFees = ({ amount, accountAddress }: FormattedValues) => {
    ...
    if (!shouldPayBridgingFee) return { result: () => Promise.resolve(undefined), unsubscribe: () => {} };

    const vftManagerProgram = new VftManagerProgram(api, CONTRACT_ADDRESS.VFT_MANAGER);
    let unsubscribe: (() => void) | undefined;

    const result = (_requestBridgingBlockHash: HexString) =>
