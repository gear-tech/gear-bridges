...
async fn run() -> AnyResult<()> {
    ...
    match cli.command {
        CliCommands::UpdateVerifierSol(args) => {
            ...
            let header_finalized = gear_api.api.rpc().chain_get_header(Some(finalized_head)).await?.expect("Finalized header");

            let (block_number, block_hash) = match args.block_number {
                None => (header_finalized.number, finalized_head),
                Some(block_number) => {
                    (block_number, gear_api.block_number_to_hash(block_number).await.context("Unable to determine hash of block with merkle root")?)
                }
            };
            ...
            for n in [0, 1, 2] {
                let block_number_to_check = (block_number as u64 / auth_set_id * (auth_set_id_target - n)) as u32;
                let Ok(block_hash) = gear_api.block_number_to_hash(block_number_to_check).await else {
                    continue;
                };

                let state = gear_api
                    .authority_set_state(Some(block_hash))
                    .await?;
                log::debug!("block_number_to_check = {block_number_to_check}");
                log::debug!("authority_set_id = {}", state.authority_set_id);
                log::debug!("authority_set_hash = {}", hex::encode(state.authority_set_hash));
