...
impl ContractVerifiers {
    ...
    pub async fn verify_vs_change(
        ...
        self.vs_change_vrf
            .verify_validator_set_change_proof(p_a, p_b, p_c, validator_set, validator_set_id)
            .from(account)
            .send()
            .await
        ...
        let validator_set_id = self
            ...
            .await
            .map_err(|_| VerifierError::ErrorDuringContractExecution)?;
        if validator_set_id != validator_set_id {
            return Ok(false);
        }
        Ok(true)
    }
    ...
    pub async fn verify_msg_sent(
        ...
        self.msg_sent_vrf
            .verify_msg_sent_proof(p_a, p_b, p_c, msg_hashes, validator_set_id)
            .from(account)
            .send()
            .await
            .map_err(|_| VerifierError::ErrorDuringContractExecution)?;

        let exp_msg_hashes = self
            .msg_sent_vrf
            .get_last_msg_hashes()
            .call()
            .await
            .map_err(|_| VerifierError::ErrorDuringContractExecution)?;
        if exp_msg_hashes != msg_hashes {
            return Ok(false);
        }
        Ok(true)
    }
