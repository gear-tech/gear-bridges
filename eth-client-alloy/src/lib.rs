...
use alloy_primitives::{Address, B256, Bytes, hex, TxHash, U256};
...
use alloy_sol_types::{sol, SolCall, SolInterface, SolEvent};
...
type ProviderType = ManagedNonceProvider<Ethereum, BoxTransport, GasEstimatorProvider<Ethereum, BoxTransport, SignerProvider<Ethereum, BoxTransport, RootProvider<Ethereum, BoxTransport>, EthereumSigner>>>;
    ...
    impl ContractVerifiers
    {
        ...
        pub async fn fetch_merkle_roots(
            ...
            let current_block : u64 = self.provider.get_block_number().await.map_err(|_|VerifierError::ErrorInHTTPTransport)?;
            ...
            match event.query().await{
                Ok(logs) => {
                    Ok(logs.iter().map(|(event, log)| (event.blockNumber.to(), event.merkleRoot, log.transaction_hash.unwrap_or_default())).collect())
                }
