...
contract MessageQueueTest is Test, Base {
    ...
    function _testnetVerifierFixture() private pure returns (bytes memory proof, uint256 canonicalBlock, bytes32 merkleRoot) {
        proof = bytes(
    ...
    function test_SubmitMerkleRootWithCanonicalProofFixture() public {
        ...
        (bytes memory proof, uint256 canonicalBlock, bytes32 merkleRoot) = _testnetVerifierFixture();
    ...
    function test_SubmitMerkleRootRejectsAliasedBlockNumber() public {
        ...
        (bool success,) = address(messageQueue).call(
            abi.encodeWithSelector(IMessageQueue.submitMerkleRoot.selector, aliasBlock, merkleRoot, proof)
        );
