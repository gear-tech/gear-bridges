...
interface IERC20Manager {
    ...
    event BridgingAccepted(address indexed to, address indexed token, uint256 amount, bytes32 sender);
...
library Packer {
    function pack(WithdrawMessage calldata message) external pure returns (bytes memory) {
        return abi.encodePacked(message.receiver, message.token, message.amount, message.sender);
    }
}
