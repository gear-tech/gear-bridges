...
pub fn spawn_receiver(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let mut updates =
                match utils::get_updates(&client_http, &beacon_endpoint, period, 1).await {
                    Ok(updates) => updates,
                    Err(e) => {
...
pub async fn try_to_apply(
    ...
    let (_message_id, payload, _value) = match listener.reply_bytes_on(message_id).await {
        Ok(result) => result,
        Err(e) => {
